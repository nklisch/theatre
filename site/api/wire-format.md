...
```json
{"type": "handshake", "version": "0.4.0", "godot_version": "4.3.stable", "project": "my-game"}
```

The server responds:

```json
{"type": "handshake_ack", "version": "0.4.0"}
```

If versions are incompatible, the server sends:

```json
{"type": "handshake_reject", "reason": "Version mismatch: server 0.4.0, addon 0.0.9"}
```
