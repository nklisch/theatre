...
use spectator_server::{config, tcp};
...
pub async fn run(tool: &str, json_arg: Option<&str>) -> Result<()> {
    ...
    let _ = tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::from_default_env()
                .add_directive("spectator=warn".parse().expect("valid directive")),
        )
        .try_init();
    ...
    if let Err(e) = tcp::connect_once(&state, resolved_port).await {
        let error = serde_json::json!({
            ...
            "message": e.to_string(),
            "hint": format!(
                "Ensure the Godot project is running with the Spectator addon active on port {resolved_port}."
            )
        });
        println!("{error}");
        std::process::exit(1);
