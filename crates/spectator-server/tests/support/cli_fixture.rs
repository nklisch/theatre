#![allow(dead_code)]
/// CLI test fixture: shells out to the `spectator` binary against a live Godot process.
...
/// Fixture that manages a live Godot process and shells out to the spectator binary.
...
impl SpectatorCliFixture {
    ...
    pub fn run(&self, tool: &str, params: Value) -> anyhow::Result<CliResult> {
        ...
        let output = Command::new(bin)
            ...
            .output()
            .map_err(|e| anyhow::anyhow!("Failed to spawn spectator binary: {e}"))?;
        ...
        let json: Value = serde_json::from_str(stdout.trim()).map_err(|e| {
            anyhow::anyhow!(
                "spectator output is not valid JSON (exit {exit_code}): {e}\nstdout: {stdout}"
            )
        })?;
