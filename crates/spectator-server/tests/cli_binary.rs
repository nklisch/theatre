/// Integration tests for the `spectator` binary CLI mode.
///
/// These tests exercise the binary's command-line interface directly,
/// using `env!("CARGO_BIN_EXE_spectator")` to locate the built binary.
use std::process::Command;

fn spectator_bin() -> &'static str {
    env!("CARGO_BIN_EXE_spectator")
}

/// Run the spectator binary with the given args and return (status code, stdout, stderr).
fn run(args: &[&str]) -> (i32, String, String) {
    let output = Command::new(spectator_bin())
        .args(args)
        .output()
        .expect("failed to run spectator binary");
...
/// `spectator --version` prints JSON to stdout and exits 0.
#[test]
...
/// `spectator -V` also prints version JSON.
#[test]
...
/// `spectator` with no args exits with code 1 and prints usage to stderr.
#[test]
...
/// `spectator unknown_tool '{}'` exits with code 2 and stdout has error JSON.
#[test]
...
/// `spectator spatial_snapshot 'not json'` exits with code 2 and stdout has error JSON.
#[test]
...
/// `spectator spatial_snapshot '{}'` with no Godot running exits code 1 with connection error JSON.
#[test]
