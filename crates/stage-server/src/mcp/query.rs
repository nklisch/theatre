...
fn query_result_entry(r: &NearestResult, perspective: &Perspective) -> serde_json::Value {
    ...
    serde_json::json!({
        "path": r.path,
        "distance": stage_core::types::round1(r.distance),
        "bearing": rel.bearing,
        "class": r.class,
    })
}
...
pub async fn build_relationship_response(
    ...
    let mut result = serde_json::json!({
        "distance": stage_core::types::round1(distance),
    ...
    if let Some(elev) = &rel_a_to_b.elevation {
        use stage_core::types::Elevation;
    ...
    if let Some(nav) = nav_distance {
        result["nav_distance"] = serde_json::json!(stage_core::types::round1(nav));
    }
