...
- Environment-dependent tests may use `#[ignore]`, but implementation delivery
  explicitly invokes them. The complete verification commands are in
  `.work/CONVENTIONS.md`.

## Common Violations

- Treating a passing ordinary workspace run as proof that ignored engine
  journeys ran; invoke those journeys explicitly.
