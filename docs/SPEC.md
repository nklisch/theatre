...
| Frame capture (dashcam) | GDExtension | Must run every _physics_process |
...
| `recording_marker` | `action`, `label`, `frame` | Marker data / clip_id |
| `recording_markers` | `clip_id` | Markers for a saved clip |
| `recording_list` | — | Saved clips metadata |
| `recording_delete` | `clip_id` | Ack with clip_id |
| `recording_resolve_path` | — | Resolved storage path |
| `dashcam_status` | — | Dashcam state and config |
| `dashcam_flush` | `marker_label` | clip_id of saved clip |
| `dashcam_config` | config fields | Ack |
...
Clips are saved automatically by the dashcam ring buffer when an interesting moment is marked. The agent calls `clips(action: "add_marker")` or presses F9 in-game; the dashcam saves a clip containing the pre- and post-window around that moment. The agent then analyzes saved clips with `snapshot_at`, `query_range`, `diff_frames`, `find_event`.
...
2. On trigger (marker, F9, agent `save`), the pre- and post-window frames are written to a new SQLite file
...
Clip queries (`snapshot_at`, `query_range`, `diff_frames`, `find_event`) are SQL queries against the SQLite database. The server deserializes frame data from MessagePack on demand.
...
| Dashcam frame capture | < 1ms | Must fit in _physics_process |
...
Clips capture game state (node paths, property values, positions). They are stored locally. No data is sent to external services. Clips are as private as the game project itself.
