...
- [ ] Cargo workspace with 4 crates (`spectator-server`, `spectator-godot`, `spectator-protocol`, `spectator-core`)
- [ ] `spectator-server` compiles as a binary, starts tokio runtime, prints "waiting for connection"
- [ ] `spectator-godot` compiles as cdylib, loads in Godot 4.2+ without errors
- [ ] GDExtension manifest (`.gdextension`) with at least Linux debug target
- [ ] GDScript `plugin.gd` (EditorPlugin) — enables/disables cleanly, registers autoload
- [ ] GDScript `runtime.gd` (autoload) — instantiates GDExtension `SpectatorTCPServer` node
- [ ] `SpectatorTCPServer` listens on configurable port (default 9077, localhost only)
- [ ] `spectator-server` connects to addon via TCP
- [ ] Handshake exchange: addon sends version/dimensions/project, server ACKs
- [ ] `spectator-protocol` crate defines handshake message types with serde
- [ ] Length-prefixed JSON codec (4-byte big-endian + JSON) working in both directions
- [ ] Basic reconnection: server retries every 2s when connection drops
- [ ] `plugin.cfg` with metadata
- [ ] `CLAUDE.md` with repo conventions for agents working in this codebase
- [ ] CI: `cargo fmt --check`, `cargo clippy --workspace`, `cargo test --workspace`, `cargo build --release`
...
- [ ] Project Settings integration (Godot editor):
  - [ ] `spectator/connection/port`
  - [ ] `spectator/connection/auto_start`
  - [ ] `spectator/recording/storage_path`, `max_frames`, `capture_interval`
  - [ ] `spectator/display/show_agent_notifications`, `show_recording_indicator`
  - [ ] `spectator/keybindings/toggle_recording`, `drop_marker`, `toggle_pause`
  - [ ] `spectator/tracking/default_static_patterns`, `token_hard_cap`
...
- [ ] Activity feed protocol:
  - [ ] Server sends `activity_log` push events to addon via TCP
  - [ ] Addon routes to dock for display
- [ ] Agent action notifications in-game:
  - [ ] Toast popups on CanvasLayer for `spatial_action` operations
  - [ ] Auto-dismiss after 3 seconds
  - [ ] Stackable (max 3 visible)
  - [ ] Disableable in Project Settings
...
- [ ] Markers:
  - [ ] Human markers via F9 / dock button (with optional text note)
...
- [ ] Keyboard shortcuts:
  - [ ] F8 — toggle recording (via `runtime.gd` `_shortcut_input`)
  - [ ] F9 — drop marker
  - [ ] F10 — toggle pause
  - [ ] Visual feedback: recording indicator (red dot), marker flash, pause overlay
...
## Milestone 11: Distribution & Polish
